...
CREATE TABLE IF NOT EXISTS `banco`.`cuentabancarias` (
  ...
  INDEX `fk_CuentaBancaria_Cliente1_idx` (`idCliente` ASC) ,
...
CREATE TABLE IF NOT EXISTS `banco`.`movimientos` (
  ...
  INDEX `fk_Movimiento_Cliente1_idx` (`idCliente` ASC) ,
...
CREATE TABLE IF NOT EXISTS `banco`.`prestamos` (
  ...
  UNIQUE INDEX `cantidadMensual_UNIQUE` (`cantidadMensual` ASC) ,
  INDEX `fk_Prestamos_Cliente1_idx` (`idCliente` ASC),
...
CREATE TABLE IF NOT EXISTS `banco`.`pagos` (
  ...
  INDEX `fk_Pago_Prestamos1_idx` (`idCliente` ASC, `idPrestamo` ASC) ,
  INDEX `fk_pago_movimiento1_idx` (`idMovimiento` ASC, `idCliente` ASC) ,
...
CREATE TABLE IF NOT EXISTS `banco`.`perfiles` (
  ...
  INDEX `fk_Perfil_Cliente_idx` (`idCliente` ASC),
  INDEX `fk_Perfil_Cliente1_idx` (`idPareja` ASC) ,
...
BEGIN
select nombre, apellidos, telefono, prestamos.* from clientes inner join prestamos on uid=idcliente where fechaOferta like "2023-05-%";
END$$
